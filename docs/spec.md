...
- **絵文字**: 📖🧠🖥️
- **キャッチコピー**: 「散らばったAI会話・メモ・資料を、再利用できる知識資産に変える」

---

## 2026-06-12 Product Update: App-first

当初はObsidianプラグインを中心に設計した。
しかし、現場では開発者本人がObsidianを日常的に使えていない。

そのため、最初の完成形をObsidianプラグインにすると、完成しても日常利用されない危険がある。

新方針:

- まず独立したローカルアプリとして完成させる
- Obsidianは必須ではなく、読み込み元・書き出し先のひとつにする
- `app/` を追加し、`service/` がブラウザUIを配信する
- 将来的に `.dmg` / `.exe` に包む
- `plugin/` はObsidian adapterとして残す

詳細: `docs/product_pivot_2026-06-12.md`
...
## Concept
### 3秒説明（たとえ付き）
ローカルフォルダにあるAI会話・メモ・資料を、**AIが"図書館司書＋編集者"として整理し直す**アプリ。
- 長すぎる本は「章」に分ける
- 断片メモは「テーマ別ノート」にまとめる
- 価値が薄いものは「ゴミ箱候補」に分ける
...
| UI方針 | **App-first**。まず `app/` のブラウザUIをserviceから配信し、Obsidianはadapter扱い | 2026-06-12 |
| 統一方針 | app・plugin・service・共通コードをすべてTypeScript/JavaScript系で統一 | 2026-02-26 / 2026-06-12 |
...
│
├── app/                           🖥️ 最初に完成させるローカルWebアプリ
│   ├── index.html
│   ├── styles.css
│   └── app.js
│
├── plugin/                        🧩 Obsidian連携adapter
│   └── src/
...
### 各部層の役割（図書館でたとえると）
- **app**: 最初に使うアプリの玄関
- **plugin**: Obsidian連携用の追加カウンター
- **service**: 山間に動く司書・実際の処理エンジン
- **shared**: カウンターと司書が両方使う「共通言語」（型定義）

### ✨ serviceの自動起動設計（致命的弱点の克服）

**旧方針：プラグインを入れるだけで全体が動く**

2026-06-12以降の新方針:

- まず `npm run start:app` でローカルアプリを起動する
- ブラウザで `http://127.0.0.1:3000/` を開く
- Obsidianプラグインは後続adapterとして扱う
- 日常利用が固まってからObsidian内UIを強化する

旧Obsidian方針は以下の通り、後続機能として残す。
